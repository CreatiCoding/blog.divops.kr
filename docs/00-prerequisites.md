...
| 항목 | 값 |
|------|-----|
| Application name | `blog.divops.kr` |
| Homepage URL | `https://blog.divops.kr` |
| Authorization callback URL | `https://blog.divops.kr/api/auth/callback/github` |
...
도메인 관리 패널에서 `blog.divops.kr`이 Dokploy 서버를 가리키도록 설정합니다.

| 타입 | 호스트 | 값 |
|------|--------|-----|
| A | `blog` | Dokploy 서버 IP |
...
```
postgresql://<user>:<password>@<host>:5432/blog
```
