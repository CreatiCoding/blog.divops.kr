    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">{allPosts.length} posts total</p>
        </div>
        <Link
          href="/admin/write"
          className="inline-flex items-center gap-1.5 px-4 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg cursor-pointer hover:bg-gray-800 transition-colors shadow-sm"
        >
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-100">
              ...
              <th className="text-left px-5 py-3 text-xs font-medium text-gray-400 uppercase tracking-wider">Category</th>
            ...
            {allPosts.map((post, i) => (
              <tr
                ...
                className={`group hover:bg-gray-50/80 transition-colors ${
                  i < allPosts.length - 1 ? 'border-b border-gray-50' : ''
                }`}
              >
                ...
                <td className="px-5 py-3.5">
                  {post.category ? (
                    <span className="inline-flex items-center text-xs font-medium text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md">
                      {post.category}
                    </span>
                  ) : (
